...
const TARGET = `${DATE}T10:15:00.000Z`;
...
const heatmap = [9, 10, 11].flatMap((hour, row) =>
...
export default function TileMotionVisualReviewPage() {
  const [selected, setSelected] = useState<Set<string>>(() => new Set());

  useEffect(() => {
    const selectTimer = window.setTimeout(() => setSelected(new Set([TARGET])), 1000);
    const deselectTimer = window.setTimeout(() => setSelected(new Set()), 2000);
    return () => {
  ...
  return (
    <main className="min-h-svh px-5 py-8 sm:px-8">
      <section className="mx-auto max-w-4xl">
        <p className="text-xs uppercase tracking-[0.16em] text-primary/70">Synk motion review</p>
        <h1 className="mt-2 text-xl font-semibold">Availability tile selection</h1>
        <p className="mt-1 mb-6 text-sm text-muted-foreground">
          The 10:15 tile selects at 1s and deselects at 2s.
        </p>
